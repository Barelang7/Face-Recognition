...
import time
# from modul import execute_unix
import pickle
# import subprocess
...
class recognizer:
    ...
    def __init__(self):
        ...
        self.nn_model = pickle.load(open("hasilklasifikasinn.sav", 'rb'))
    ...
    def face_neuralnetwork_classifier(self, X):
        """Methods takes in preprocessed images ,classifies and returns predicted Class label and probability"""
        # predict
        yhat = self.nn_model.predict(X)
        self.label = yhat[0]
        yhat_prob = self.nn_model.predict_proba(X)*100
        ...
        self.label = predicted_class_label[0]
        return self.label, str(probability)

    # def execute_unix(self):

    #     data = "halo kamu " + self.label + " ya"
    #     c = 'espeak -vid+f3 -k5 -s1510 --punct="<characters>" "%s" 2>>/dev/null' % data
        ...
        while True:
            ...
            if result:
                for person in result:
                    ...
                    self.label, probability = self.face_neuralnetwork_classifier(X)
